...
const optionPattern = /(?:^|\n)([1-9][0-9]*)\s*\.(?=\s*\S)/gu;
// Corrections for blocks whose source text can't be recovered from the PDF
// text layer: question stems rendered as images, and sign numbers printed as
// "1." that the option pattern leaves with a trailing period.
const sameSpeedLimitQuestion = "Ո՞ր նշանն է արագության սահմանափակում մտցնում։";
const sourceOverrides = {
  "խումբ-4-հայերեն-page-10-q057": {
    options: { 1: "1", 2: "2", 3: "3" },
  },
  "խումբ-4-հայերեն-page-16-q093": {
    question: sameSpeedLimitQuestion,
    image: "/question-images/խումբ-4-հայերեն-page-16-image-05.png",
  },
  "խումբ-8-հայերեն-page-12-q070": {
    question: sameSpeedLimitQuestion,
    image: "/question-images/խումբ-8-հայերեն-page-12-image-01.png",
  },
};
const visualLanguage =
  /նկար|իրադրությ|նշված (?:տեղ|գոտ|հետագծ)|ավտոմոբիլ|տրանսպորտային միջոցի վարորդ|ուղղությ(?:ամ|ո)ւն|նշան|գծանշ/u;
function classify(question, file) {
  const rules = [
    [
      "Առաջին օգնություն",
      /օգնություն|տուժած|վնասված|արյունահոս|վերք|կոտրված|շնչառ|այրված|ցնցում/u,
    ],
    ["Ճանապարհային նշաններ", /նշան/u],
    ["Գծանշումներ", /գծանշ/u],
    ["Երկաթուղային գծանցներ", /երկաթուղ/u],
    ["Հետիոտներ", /հետիոտ/u],
    ["Հեծանվորդներ", /հեծանվ/u],
    ["Տրամվայ", /տրամվայ/u],
    ["Կանգառ", /կանգառ/u],
    ["Կայանում", /կայան/u],
    ["Վազանց", /վազանց/u],
    ...
    ["Հետադարձ", /հետադարձ/u],
    ["Շրջադարձ", /շրջադարձ/u],
    ["Խաչմերուկներ", /խաչմերուկ/u],
    ["Առաջնահերթություն", /առաջնահերթ/u],
    ["Վերադասավորում", /վերադաս/u],
    ["Մերձակա տարածք", /բակ|մերձակա/u],
  ];
  ...
  const matches = rules
    .filter(([, test]) => test.test(question))
...
function pageCount(pdfPath) {
  return Number(
    shell("pdfinfo", [pdfPath]).match(/^Pages:\s+(\d+)/m)?.[1] || 0,
  );
}
...
function xmlLines(pdfPath, page, side) {
  ...
  // Armenian glyphs of different heights on one visual line report different
  // yMin values, so words are grouped by their shared bottom edge instead.
  const lines = [];
  for (const word of words) {
    const line = lines.at(-1);
    if (line && word.baseline - line.baseline <= 2) {
      line.words.push(word);
      line.y = Math.min(line.y, word.y);
    } else lines.push({ y: word.y, baseline: word.baseline, words: [word] });
  }
  return lines.map(({ y, words: wordsOnLine }) => ({
    y,
    text: wordsOnLine
      .sort((a, b) => a.x - b.x)
      .map((word) => word.text)
...
function parseBlock(lines, context) {
  ...
  if (!matches.length) {
    return {
      warning: {
        ...context,
        reason: "no-detectable-options",
        extractedText: normalize(lines),
        sourceAnswer: Number(answer[1]),
  ...
  const question =
    override?.question ??
    normalize(beforeAnswer.slice(0, matches[0].index).split("\n"));
  const options = matches.map((match, i) => ({
    sourceIndex: Number(match[1]),
    text:
      override?.options?.[match[1]] ??
      normalize(
        beforeAnswer
          .slice(
  ...
  const matchingImage = context.images.find(
    (image) =>
      image.side === context.side &&
      image.y >= context.startY - 18 &&
      image.y <= context.endY + 8,
  );
  const visual = override?.image
    ? { type: "image", src: override.image, page: context.page }
    : matchingImage
      ? { type: "image", src: matchingImage.src, page: context.page }
      : imageDependent
        ? {
            type: "page",
            src: `/question-pages/${context.sourceId}-page-${String(context.page).padStart(2, "0")}.jpg`,
            page: context.page,
          }
        : undefined;
  return {
    question: {
      id,
      question,
      originalOptions: options.map((option) => ({
        id: `${id}-option-${option.sourceIndex}`,
        ...option,
        generated: false,
      })),
    ...
    warnings: reasons.map((reason) => ({
      ...
      sourceAnswer,
      visualReviewRequired: imageDependent,
    })),
  };
}
function renderPage(pdfPath, sourceId, page) {
  const target = join(
    visualDir,
    `${sourceId}-page-${String(page).padStart(2, "0")}`,
  );
  if (!existsSync(`${target}.jpg`))
    shell("pdftoppm", [
      "-f",
      String(page),
      "-l",
...
function extractPageImages(pdfPath, sourceId, page) {
  ...
  const xml = existsSync(`${temporaryBase}.xml`)
    ? readFileSync(`${temporaryBase}.xml`, "utf8")
    : "";
  return [
    ...xml.matchAll(
    ...
    return {
      x: Number(match[2]) / 1.5,
      y: Number(match[1]) / 1.5,
      side: Number(match[2]) < 459 ? "left" : "right",
      src: `/question-images/${targetName}`,
    };
  });
}
...
for (const file of pdfs) {
  ...
  const sourceId = basename(file, ".pdf")
    .replace(/[^\p{L}\p{N}]+/gu, "-")
    .replace(/^-|-$/g, "")
    .toLowerCase();
  let questionIndex = 0;
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    for (const side of ["left", "right"]) {
      ...
      for (const line of lines) {
        current.push(line);
        if (answerPattern.test(line.text)) {
          blocks.push(current);
          current = [];
          answerKeys++;
        }
      ...
      if (normalize(current))
        fileWarnings.push({
          ...
          page,
          side,
          reason: "unclosed-content-after-answer-marker",
          extractedText: normalize(current),
        });
      for (const block of blocks) {
        ...
        if (result.question) {
          if (result.question.visual) {
            imageQuestions++;
            renderPage(pdfPath, sourceId, page);
          }
          ...
          fileWarnings.push(result.warning);
        }
      }
    }
  }
  inventory.push({
    ...
    path: `docs/${file}`,
    pages,
    extractionStatus: fileWarnings.length
      ? "completed-with-warnings"
      : "completed",
    questionsFound: questionIndex,
    imageDependentQuestions: imageQuestions,
    answerKeysFound: answerKeys,
    parsingWarnings: fileWarnings.length,
    unresolvedQuestions: fileWarnings.filter((x) =>
      /missing|no-detectable|answer-index|fewer|empty/.test(x.reason),
    ).length,
  });
}
...
for (const group of normalizedGroups.values())
  if (group.length > 1) {
    ...
    for (const question of group)
      question.duplicateGroupId = `duplicate-${String(duplicateNo).padStart(3, "0")}`;
  }

const sourceOptions = questions.reduce(
  (count, q) => count + q.originalOptions.length,
  0,
);
const report = {
  ...
  questionsWithExtractedImages: questions.filter(
    (q) => q.visual?.type === "image",
  ).length,
  questionsWithPageVisualFallback: questions.filter(
    (q) => q.visual?.type === "page",
  ).length,
  questionsWithFourSourceOptions: questions.filter(
    (q) => q.originalOptions.length === 4,
  ).length,
  questionsWithGeneratedDistractors: 0,
  totalSourceOptions: sourceOptions,
  questionsWith2SourceOptions: questions.filter(
    (q) => q.originalOptions.length === 2,
  ).length,
  questionsWith3SourceOptions: questions.filter(
    (q) => q.originalOptions.length === 3,
  ).length,
  questionsWith4SourceOptions: questions.filter(
    (q) => q.originalOptions.length === 4,
  ).length,
  questionsRequiringGeneratedDistractors: questions.filter(
    (q) => q.originalOptions.length >= 2 && q.originalOptions.length < 4,
  ).length,
...
writeFileSync(
  join(reportsDir, "question-extraction-report.json"),
  `${JSON.stringify(report, null, 2)}\n`,
);
writeFileSync(
  join(reportsDir, "question-review.json"),
