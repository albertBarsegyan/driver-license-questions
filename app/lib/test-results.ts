...
function openDatabase(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!database.objectStoreNames.contains(STORE_NAME)) database.createObjectStore(STORE_NAME, { keyPath: "testId" });
    };
...
export async function getTestResults(): Promise<Record<string, TestResult>> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      database.close();
      resolve(Object.fromEntries(request.result.map((result: TestResult) => [result.testId, result])));
    };
  });
}
