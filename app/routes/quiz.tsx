...
function buildMixedTests(items: QuizItem[], scope: string): TestGroup[] {
  const medical = items.filter((item) => sourceGroupNumber(item.source) === MEDICAL_GROUP);
  const groups = Array.from({ length: 9 }, (_, index) => items.filter((item) => sourceGroupNumber(item.source) === index + 1));
  const nonMedical = groups.flat();
  if (!medical.length || groups.some((group) => !group.length)) return buildSequentialTests(items, scope);
  ...
  const testCount = Math.max(Math.ceil(nonMedical.length / (QUESTIONS_PER_TEST - 1)), medical.length);
  ...
  for (let index = 0; index < testCount * extraPerTest; index++) tests[Math.floor(index / extraPerTest)].push(remaining[index % remaining.length]);
  for (let index = 0; index < testCount; index++) tests[index].push(medical[index % medical.length]);

  return tests.map((testItems, index) => ({ id: `${scope}:test-${index + 1}`, number: index + 1, items: testItems }));
}
...
export default function Quiz() {
  ...
  const tests = useMemo<TestGroup[]>(() => {
    ...
    return !source && !category ? buildMixedTests(items, scope) : buildSequentialTests(items, scope);
  }, [source, category]);
