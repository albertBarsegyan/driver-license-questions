...
export default function Category({ params }: Route.ComponentProps) {
  ...
  return (
    <section>
      ...
      <Link
        className="button"
        to={`/quiz?category=${encodeURIComponent(category)}`}
      >
        Սկսել թեստը
      </Link>
      <div className="question-list category-question-list">
        {list.map((question) => {
          return (
            <article className="category-question" key={question.id}>
              <Link
                to={`/questions/${question.id}?category=${encodeURIComponent(category)}`}
              >
                {question.question}
              </Link>
              <small>
                {question.source.file} · էջ {question.source.page}
              </small>
            </article>
          );
        })}
